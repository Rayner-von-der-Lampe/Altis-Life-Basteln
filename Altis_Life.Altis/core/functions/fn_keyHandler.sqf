...
switch (_code) do
{
	...
	case 19:
	{
		...
		//Shift+R Kabelbinder ( Zivilisten können fesseln )
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					if(_locked == 0) then {
						...
						hint composeText [ image "icons\lock.paa", " Du hast dein Haus Abgeschlossen." ];
					} else {
						_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
						_veh animate [format["door_%1_rot",_door],1];
						hint composeText [ image "icons\unlock.paa", " Du hast dein Haus Aufgeschlossen." ];
					};
				};
			} else {
				_locked = locked _veh;
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						hint composeText [ image "icons\unlock.paa", "Du hast dein Fahrzeug aufgeschlossen." ];
						...
						hint composeText [ image "icons\lock.paa", " Du hast dein Fahrzeug abgeschlossen." ];
