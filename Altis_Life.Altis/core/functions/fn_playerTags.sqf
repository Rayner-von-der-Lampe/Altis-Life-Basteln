	...
	if(player distance _x < 30 && isPlayer _x && _x != player) then {
		...
		if(!_visible) then {
			...
			switch (_x getVariable["coplevel", 0]) do
				...
				{
					case (1) : {_name = format["[*] %1", name _x];_icon = MISSION_ROOT + "images\icon_recruit.paa";};
          			case (2) : {_name = format["[**] %1", name _x];_icon = MISSION_ROOT + "images\icon_officer.paa";};
          			case (3) : {_name = format["[***] %1", name _x];_icon = MISSION_ROOT + "images\icon_sergeant.paa";};
          			case (4) : {_name = format["[****] %1", name _x];_icon = MISSION_ROOT + "images\icon_lieutenant.paa";};
          			case (5) : {_name = format["[*****] %1", name _x];_icon = MISSION_ROOT + "images\icon_captain.paa";};
          			case (6) : {_name = format["[SEK] %1", name _x];_icon = MISSION_ROOT + "images\icon_deputychief.paa";};
					case (7) : {_name = format["[******] %1", name _x];_icon = MISSION_ROOT + "images\icon_chief.paa";};
