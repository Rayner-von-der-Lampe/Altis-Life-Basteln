...
_random =  ["lwatch","cables","woodleg","shoki","luckycookie","calc"];// items, which can be found
_randomitem = _random select (floor (random (count _random))); // making the items random
switch(true) do {
	case (_zone in ["apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","meth_1","lsd_1","heroin_1","cocaine_1","weed_1","Birnen_1","Birnen_2","Birnen_3","Birnen_4","trau_1","trau_2","trau_3","trau_3","flasch_1","flasch_2","flasch_3","flasch_4","flasch_5","musch_1","musch_2","musch_3"]): {_randomitem; _val = 1;}; // switch from _gather to _randomitem to make script kepping easy
...
{
	_itemName = [([_randomitem,0] call life_fnc_varHandle)] call life_fnc_varToStr;//  same as _gather before
