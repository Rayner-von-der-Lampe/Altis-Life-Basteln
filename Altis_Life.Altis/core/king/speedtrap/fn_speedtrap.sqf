...
while{true} do
{
    ...
    _var_speedtrapdetector = ["speedtrapdetector",0] call life_fnc_varHandle;
            ...
            if(_distance < _trap_distance && _speed > (_speedlimit + _trap_speed_tolerance)) then
            {
                ...
                if(!_visible) then
                {
                    ...
                    if( (_last_traptime == 0) || ((_last_traptime + _blocktime_trap) < time) ) then
                    {
						...
						[[getPlayerUID player,name player,"102"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
                        _trapped_infos = [];
