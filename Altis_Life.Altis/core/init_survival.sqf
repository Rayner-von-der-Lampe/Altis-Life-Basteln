...
[] spawn
{
	...
	while{true} do
	{
		...
		_load = round(_cfg / 8);
		if (backpack player != "") then { _load = 40; };
		if (backpack player != "" && license_civ_addInv) then { _load = 60; }; //<-----Add Backpackweight If license is available
...
// [] spawn
// {
// _illegalmarkers = ["heroin_1","cocaine_1","weed_1"];
