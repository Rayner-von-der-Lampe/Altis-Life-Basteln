...
if (isnil "IL_Variables") then
{
	...
	IL_Supported_Vehicles_HEMTT = ["B_Truck_01_mover_F", "B_Truck_01_transport_F"];
	...
	IL_Para_Drop_Vehicles = IL_Supported_Vehicles_MOHAWK + IL_Supported_Vehicles_C130J + IL_Supported_Vehicles_C17 + IL_Supported_Vehicles_CHINOOK;
	...
	IL_Supported_Veh_Ammo = ["Land_WaterTank_F", "Box_NATO_AmmoVeh_F","Land_Cargo20_blue_F"];
	IL_Supported_Barrel = [""];
	IL_Supported_Tank = [""];
	...
	IL_Supported_Cargo_Veh_HEMTT = IL_Supported_Cargo_Veh_Kamaz + IL_Supported_OFFROAD + IL_Supported_SUV + IL_Supported_Hunter + IL_Supported_Ifrit;
...
//	END IL_Init_Veh
	...
	IL_Init_Box =
	{
		...
		{
			if ((_obj_type == "Land_WaterTank_F") OR (_obj_type == "Land_Cargo20_blue_F")) then
			{
				_obj setVariable["slots", 3, true];
		...
		{
			{
				...
				else
				{
					...
					{
						[_v, Format ["Beginne ""%1"" auf ""%2"" zu laden", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						_done = true;
						_counter = (_box_num);
						...
						{
							[_v, Format ["""%1"" geladen auf ""%2"". Freier Platz: ""%3"".", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), abs(_slot_num - _counter)]] call IL_Vehicle_Chat;
						}
						else
						{
							[_v, Format ["""%1"" geladen auf ""%2"" Der Haenger ist Voll.", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						};
					}
					else
					{
		...
		{
			{
				...
				if (_x getVariable "doors" == _doors) then
				{
					...
					if (((_x getVariable "attachedTruck") == _v) && ((_x getVariable "attachedPos") == (_counter)) && (_counter < 0) && !_done) then
					{
						...
						[_v, Format ["""%1"" geladen auf ""%2"". Freier Platz: ""%3"".", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), _free_slots]] call IL_Vehicle_Chat;
						sleep 1;
					};
				};
				if (_done) exitWith {};
...
if (_obj_main_type in IL_Supported_Vehicles_HEMTT) then
{
	...
	_obj_main addAction [
	"<img image='IgiLoad\images\load.paa' /><t color=""#007f0e"">  Belade Fahrzeug</t>",
	{
	...
	_obj_main addAction [
	"<img image='IgiLoad\images\unload.paa' /><t color=""#ff0000"">  lade Fahrzeug ab</t>",
	{
	...
	_obj_main addAction [
	"<t color=""#0000ff"">Rampenbeladung aktivieren</t>",
	{
		(_this select 0) setVariable["can_outside", true, true];;
	},[],IL_Action_S_Priority,false,true,"",
	"((driver _target == _this) && !(_target getVariable 'can_outside') && IL_Can_Outside)"
	...
	_obj_main addAction [
	"<t color=""#0000ff"">Rampenbeladung deaktivieren</t>",
	{
		(_this select 0) setVariable["can_outside", false, true];;
	},[],IL_Action_S_Priority,false,true,"",
	"((driver _target == _this) && (_target getVariable 'can_outside') && IL_Can_Outside)"
...
{
	Player globalChat Format["Objekt: ""%1"" wird nicht unterstuetzt.", _obj_main_type];
};
